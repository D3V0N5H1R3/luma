...
1. **Reproduce** the bug with a minimal Luma program and the DAP request sequence that misbehaves (set breakpoints, continue, step over/into/out, stack trace, scopes, variables, evaluate, threads). Confirm the failure, ideally as a C++ test in `debugger/tests/`.
2. **Isolate the component** where the bug occurs by tracing through the debugger:
    - Protocol transport (`dap_transport.*`, `dap_protocol_handler.hpp`, `shared/protocol/`) — Content-Length framing or request dispatch wrong?
    ...
    - Breakpoints (`breakpoint_manager.*`, `compiled_breakpoint.*`, `line_breakpoint_manager.*`, `exception_breakpoint_settings.hpp`, `dap_breakpoint_validator.*`) — pending vs resolved entries, conditions, hit counts, logpoints, or exception filters (caught/uncaught) wrong?
    ...
    - Variable inspection (`variable_inspector.*`, `variable_reference_registry.hpp`) — scope expansion or generational reference invalidation wrong?
