...
- **In scope:** first-party sources under `core/`, `shared/`, `language-server/`, and `debugger/`, plus the Luma-level cost of stdlib operations exercised by `benchmarks/`.
- **Out of scope:** vendored code (`external/`, currently mbedtls and miniz), generated code, and build outputs (`build/`, `build-fuzz/`). Do not propose optimizations there.
