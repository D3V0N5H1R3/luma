...
1. **Reproduce** the bug with a minimal Luma document and the specific LSP request that misbehaves (diagnostics, hover, completion, signature help, go-to-definition, references, rename, semantic tokens, code actions, formatting, folding, document links). Confirm the failure, ideally as a C++ test in `language-server/tests/`.
2. **Isolate the layer** where the bug occurs by tracing through the server:
    ...
    - Analysis result indexing (`lsp_analysis_result.hpp`, `lsp_token_index.hpp`, `lsp_scope_stack.*`, `lsp_identifier_collector.*`) — token index, function body ranges, or scoped locals wrong?
    ...
    - Symbol resolution (`lsp_symbol_resolver.*`, `lsp_definition_resolver.hpp`, `lsp_symbol_lookup.hpp`, `lsp_navigation_handler.hpp`, `lsp_rename_handler.hpp`) — scope-aware lookup backing hover, definition, references, and rename wrong?
    ...
    - Code actions & quick fixes (`lsp_code_action_handler.hpp`, `lsp_code_action_builder.hpp`, `lsp_quickfix_handler.hpp`, `lsp_server_code_actions*.cpp`) — wrong or missing quick fix edit?
    ...
    - Document symbols (`lsp_symbol_handler.hpp`, `lsp_server_symbols.cpp`) — wrong outline, symbol kind, or declaration range?
    - Folding & document links (`lsp_folding_handler.hpp`, `lsp_server_folding.cpp`, `lsp_navigation_handler.hpp`, `lsp_server_navigation.cpp`, `lsp_brace_matcher.hpp`) — wrong ranges or link targets?
