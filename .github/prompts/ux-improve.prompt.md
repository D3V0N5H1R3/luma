...
3. **Establish a green baseline.** Build and test **before** changing anything, and record any pre-existing failures so they stay distinguishable from regressions:
    ...
    - Run the examples headlessly: `python scripts/run_examples.py` — the `gui_*` / `solaris_*` examples are driven with `LUMA_GUI_HEADLESS=1` and are **not** part of `ctest`, so this is the only guardrail that exercises the very surfaces you are about to change.
...
5. **Verify each change against the running interface.** After each change, exercise the surface you touched — a clean rebuild is the first checkpoint, a clean headless run the second:
    ...
    - Re-run `python scripts/run_examples.py` so every example — including the one you changed — still completes cleanly.
