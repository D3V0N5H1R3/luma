...
3. Build and run the full test suite **before** starting to establish a green baseline, and record any pre-existing failures so they stay distinguishable from regressions. Each subsystem builds and tests differently — build and test whichever ones your change touches:
    ...
    - **Zed extension** (Rust): `cargo build --release --target wasm32-wasip1` to build (it compiles to WebAssembly, so the `wasm32-wasip1` target is required), plus `npx tree-sitter generate` when the grammar changes; the tree-sitter parse-fixture tests (`node extensions/tests/parse-fixtures.js`) validate it.
