...
void register_string_ns(const EnvPtr& env) {
    ...
    env->define("String.ascii_lowercase", Value{std::string{"abcdefghijklmnopqrstuvwxyz"}},
                false);
    env->define("String.ascii_uppercase", Value{std::string{"ABCDEFGHIJKLMNOPQRSTUVWXYZ"}},
                false);
    env->define(
    ...
    env->define("String.punctuation",
                Value{std::string{R"(!"#$%&'()*+,-./:;<=>?@[\]^_`{|}~)"}}, false);
