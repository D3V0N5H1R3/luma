...
- **Independent change frequency.** Vendored-library detection (`LumaMbedTLS`) changes for entirely different reasons than compiler warning flags (`LumaCompilerFlags`). Separate files mean unrelated changes never produce diffs in the same module.
...
The vendored third-party libraries (`LumaMbedTLS.cmake`, `LumaMiniz.cmake`) are included from [`core/runtime/CMakeLists.txt`](../core/runtime/CMakeLists.txt), close to the target that links them, so the dependency wiring lives next to its consumer.
