...
| ---------------------------------------------- | ------------------------------------------------------------------------------------------- |
| [`vscode/`](vscode/)                           | VS Code extension (TypeScript) — TextMate grammar, LSP client, DAP client, run/test tasks.  |
...
Tracks feature availability across editor extensions in a comparison matrix. Shows which capabilities each extension supports (syntax highlighting, LSP, DAP, test running, etc.) and notes where one extension lags behind the other — useful for planning work to close gaps.
...
- **Feature parity as a goal.** Both extensions target the same baseline feature set. `FEATURE_PARITY.md` documents the current state and any differences in editor-specific integrations (such as test-runner UX).
