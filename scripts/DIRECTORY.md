...
| [`bump_version.py`](bump_version.py)                       | Bump the project version across all version-bearing files (`VERSION`, VS Code `package.json`, Zed `extension.toml`) simultaneously. | Manual                                                     |
...
- **Mixed toolchain.** The directory is predominantly Python; the non-Python helpers are [`container-build.sh`](container-build.sh) (POSIX shell, kept free of bashisms for CI containers), [`run_psscriptanalyzer.ps1`](run_psscriptanalyzer.ps1) and the [`pipeline/`](pipeline) prompt runners (PowerShell `*.ps1` plus their bash `*.sh` counterparts), and the [`git-hooks/`](git-hooks) Git hooks.
...
- [benchmarks/DIRECTORY.md](../benchmarks/DIRECTORY.md) — How the benchmark suite and its parse/compare scripts fit together.
