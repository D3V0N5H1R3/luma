...
Describe 'Get-EvolveKind' {
    # Pins the deterministic kind -> implementer-prompt routing the evolve runner
    # depends on. Every candidate is routed purely by its kind, so this mapping is
    # the contract that keeps a candidate from being sent to the wrong prompt. Kept
    # in lock-step with luma_evolve_kinds() in luma-pipeline.sh.

    It 'lists the four evolution kinds in order' {
        InModuleScope LumaPipeline {
            $kinds = Get-EvolveKind
            $kinds.Count | Should Be 4
            (($kinds | ForEach-Object { $_.Kind }) -join '|') | Should Be 'function|type|module|feature'
        }
    }

    It 'routes each kind to its implementer prompt' {
        InModuleScope LumaPipeline {
            $byKind = @{}
            foreach ($k in Get-EvolveKind) { $byKind[$k.Kind] = $k.Prompt }
            $byKind['function'] | Should Be 'new-stdlib-function.prompt.md'
            $byKind['type'] | Should Be 'new-stdlib-type.prompt.md'
            $byKind['module'] | Should Be 'new-stdlib-module.prompt.md'
            $byKind['feature'] | Should Be 'new-language-feature.prompt.md'
        }
    }

    It 'gives every kind a non-empty display name' {
        InModuleScope LumaPipeline {
            foreach ($k in Get-EvolveKind) {
                [string]::IsNullOrWhiteSpace($k.Name) | Should Be $false
            }
        }
    }

    It 'names prompt files that exist under .github/prompts' {
        InModuleScope LumaPipeline {
            $repoRoot = Get-LumaRepoRoot
            foreach ($k in Get-EvolveKind) {
                $promptPath = Join-Path -Path $repoRoot -ChildPath ".github/prompts/$($k.Prompt)"
