...
- Luma is a statically typed, expression-oriented, interpreted programming language for beginners — "as easy as Python, as safe as Rust." Implemented in C++20 with a bytecode compiler and stack-based VM. Status: Alpha (0.15) — language, stdlib, and interpreter feature-complete; tooling under active development.
...
- **`reference<T>` is the only reference-cycle leak vector.** Every other value type is acyclic (value semantics + closures deep-copy captures), so the sole way to build a `shared_ptr` cycle that refcounting cannot reclaim is to reassign a `reference` cell (via `Reference.set`/`update`/`swap`) to a structure that transitively holds that reference. `ReferenceLeakTracker` (`core/runtime/interpreter/reference_leak_tracker.hpp`) counts live reference cells unconditionally (two relaxed atomics per `Reference.new`/cell destruction — reference cells are never created by copy or `deep_copy`, only by `Reference.new`, so this is cheap). It is a *detector, not a collector* — it never frees, so it cannot cause a use-after-free. `LUMA_DIAGNOSE_REFERENCE_CYCLES=1` makes `execute_bytecode` (`cli_runner.cpp`) print a one-line leak summary at shutdown, evaluated *after* the VM/environment scope closes (so the thread pool is joined and only genuine cycle leaks remain). The mitigation programs use is to break the cycle by reassigning the cell (`Reference.set(r, none)`). Pinned by `tests/runtime/soak_test.cpp`; the tests deliberately break every cycle they build so they stay green under LSan. Adding a `~ReferenceValue()`/ctor hook means `ReferenceValue` is now explicitly non-copyable/non-movable (it already was, via its `recursive_mutex` member).
...
- **`DIRECTORY.md` (not `README.md`)** is the standard name for subdirectory documentation index files throughout the repo. Only the root-level `README.md` keeps that name (for GitHub rendering). All other directories use `DIRECTORY.md` for their catalogue/overview.
- **`documents/INDEX.md` line numbers drift** when documents are edited. It maps section headings to line ranges for targeted reading; any content addition or removal in a document silently invalidates the line numbers for every subsequent section in that document's entry. After editing a document listed in `INDEX.md`, verify and update its line ranges — or the index will send readers to wrong sections.
...
- Division `/` **always yields `number`** (even for two `integer` operands: `7 / 2 == 3.5`) — assigning it to an `integer` is a type error; use `//` for integer **floor division** (rounds toward −∞: `-7 // 2 == -4`). `%` stays **truncating** (its sign follows the dividend), so `//` and `%` use *different* rounding conventions — a deliberate, documented inconsistency.
...
- Anti-patterns: using `try`/`catch` for expected failures (use `result<T>`), naked `Result.unwrap`, returning sentinel values, wrapping infallible functions in `result<T>`. **Silently discarding a `result<T>` is a hard compile error** — bind it, propagate with `?`, or explicitly drop it with `_ = expr`.
- **Empty ⇒ `optional`** for total accessors: `Array.first`/`last`/`get` return `optional<T>` (`none` when empty or out of bounds), *not* `result<T>`. Bridge to a `result`-returning caller with `Optional.to_result(o, msg)?`; otherwise handle with `Optional.unwrap_or`/`match`. Migrating an old `result`-based site (`Array.get(...)?`, `… |> Result.unwrap_or(…)`) is a common breakage.
...
- Bare `spawn` outside a `task_scope` is a **compile error** (previously a warning) — unstructured fire-and-forget tasks are disallowed so every task's lifetime is scoped. Misusing a `unique` value after it is consumed is likewise a compile error for a **local** variable (still a warning when the misuse is through a parameter).
...
- **Headless interactive-input test APIs** make display/TTY-dependent code testable in CI. `Terminal.test_*` (`terminal_testing.cpp`): `test_start(keys)`, `test_feed(keys)`, `test_output()→string`, `test_remaining()→integer`, `test_stop()→string` — keys are names (`"a"`, `"enter"`, `"ctrl+c"`, `"shift+tab"`) or mouse strings (`"mouse:left_press:5:10"`). Prefer driving these from `@test` blocks so an example proves behaviour, not just "doesn't crash".
- **Example runner**: `python scripts/run_luma_examples.py` executes every program in `examples/` headlessly — Terminal via scripted keys, Console via scripted stdin — and additionally runs any `@test` blocks (without `--strict`, so lenient on W0010 but still clean). An example with no `@test` block only proves it doesn't crash. `run_luma_examples.py` discovers every `.luma` under `examples/` via `rglob`; the suite skips `multi_file_utils.luma` (an include-only helper with no `@main`) and `echo_server.luma` (a TCP server that blocks waiting for connections). Any new example that blocks on network I/O or waits indefinitely for external input must be added to the `SKIP` list in `run_luma_examples.py` with a reason, or it will time out (45 s default) and fail CI. **`ctest` runs only `tests/`, never `examples/`** — a stdlib signature/semantics change (e.g. `Array.get` `result`→`optional`) can break examples that a green `ctest` won't catch, so run `run_luma_examples.py` after any stdlib API change.
- 29 fuzz targets in `fuzz/` (authoritative list in `fuzz/CMakeLists.txt`) spanning the analysis/runtime pipeline (lexer → VM), stdlib parsers (JSON, CSV, XML, regex, compression, …), include resolver, protocol transport, and the bytecode deserializer (with round-trip oracles), using LibFuzzer with a shared harness and seed corpus.
...
- **Block-body lambdas always get a `StdlibAny` return context** during type checking (`expression_type_checker_misc.cpp` sets `current_return_type = StdlibAny` for any `{ }`-bodied lambda, ignoring the variable's declared `function(...) -> R`). This is why `?` is allowed inside them (`numeric_operator_checker.cpp` treats a `StdlibAny` return context as permissive) and why `Result.map`/`flat_map` infer `result<U>` from the mapping lambda's `arg_types[1].return_type` (mirroring `Optional.map`/`flat_map` in `stdlib_type_signatures.cpp`). The typed `Result.map_boolean`/`_integer`/`_number`/`_string` variants remain only as a fallback for block-body lambdas whose inferred return is `StdlibAny` — an expression-body lambda infers a concrete `U`, so plain `map` suffices.
...
- **Platform abstraction pattern**: Platform-specific code uses conditional declarations in headers (`#ifdef _WIN32` / `#else`) with separate `*_posix.cpp` / `*_win32.cpp` implementation files. Shared inline utilities wrap OS-specific APIs (e.g., `platform_socket.hpp` provides unified `close()`, `last_error()`, `set_non_blocking()`, `set_timeout()`; `platform_utils.hpp` provides `wstring_to_utf8()`, `safe_getenv()`, and `is_stdin_terminal()`/`is_stdout_terminal()`/`is_stderr_terminal()` for TTY detection). Use for sockets, terminal I/O, and environment access where implementations differ significantly. When the same `#ifdef _WIN32 isatty/_isatty` pair gets duplicated across call sites (CLI colour helpers, the REPL line editor, the `Terminal` stdlib module), consolidate into one `platform_utils.hpp` helper rather than leaving each site to re-derive the fallback — `core/runtime/cli/terminal.hpp`'s `stdout_is_tty()`/`stderr_is_tty()` now simply forward to it.
...
- The standalone `Color` stdlib module was **removed** as well — colour styling for terminals is already covered by the `Terminal.Color` choice type (a distinct namespaced ADT, `type_name = "Color"`, that remains and must stay distinguishable from any user-defined top-level `Color`). Do not confuse the two: grep hits for `Color` in `core`/`shared` are all `Terminal.Color` (or the choice-name-collision handling in the parser/type-checker), not the deleted module. Removal spanned the `k_registrations` catalog list in `shared/stdlib/stdlib_catalog.cpp`, module registration, type signatures, docs, examples, and the LSP/DAP catalogs.
- The vestigial `widget` built-in type (a leftover of the removed GraphicalUi/Solaris subsystem — dictionary-backed at runtime with no module, functions, or value kind) was **removed** from the lexer `k_builtin_type_identifiers` (kept sorted for `binary_search`), the type checker (`named_record_types` + the `widget↔dictionary` coercion in `is_assignable`), the stdlib catalog (`named::widget()` factory + `ReturnTypeDesc widget` field), the LSP hover/keyword catalogs, the editor grammars (regenerate `builtin-types.ts` with `generate-all.py` and the tree-sitter parser with `tree-sitter generate`), and the docs. The MODULE_DEVELOPMENT_KIT's "Widgets" template module and example `Widget` product records are unrelated and stay.
- `Math.log(base, value)` was **removed** in favour of `Math.log_base(value, base)` — the two computed the identical `ln(value)/ln(base)` with *reversed* argument order (a beginner footgun). `Math.log_e`/`log_10`/`log_2`/`log_base` and `Log.log` are unrelated and remain. `KeyValueStore.transaction(store, apply)` was added (closure-based, all-or-nothing: commit persists a file-backed store, `failure` rolls back with nothing written).
...
- **Pre-existing test baselines**: Before refactoring, capture a fresh `ctest` baseline (exact test count + any known failures) so regressions stay distinguishable — don't trust a hardcoded number, the suite count drifts as tests are added. `dap_integration_test` needs loopback networking, so it may be skipped or fail under offline CI.
...
- **Removing a feature leaves dead plumbing on the *other* side of the interface**: deleting a *consumer* subsystem (e.g. the DAP data-breakpoint feature) can leave the *producer*-side hooks coherent-looking but permanently dead — the VM still carried a `DataBreakpointHook` alias, a `data_breakpoint_hook` field on `DebugCallbacks`, a `set_data_breakpoint_hook` setter, `notify_*_data_breakpoint` calls fired from `op_set_local`/`op_set_global`, and hook propagation in `propagate_debug_hooks`, none of which any caller reached after the DAP removal (so the hot-path `shared_lock` on every variable write was pure overhead). After removing a feature, sweep the far side of its interface for now-unreachable setters/fields/notify paths — a plain build stays green because dead-but-valid code still compiles. Grep the feature's whole term set (both `snake_case` and `camelCase` spellings) across `core`/`language-server`/`debugger`/`tests` to confirm zero residual references.
