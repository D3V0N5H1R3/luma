#include "stdlib/stdlib_catalog_internal.hpp"

namespace luma::stdlib::detail {

void register_json_functions(std::vector<FunctionSpec>& specs, const ModuleBuilder& m,
                             const ParamShorthands& p) {
    append_specs(
        specs,
        {
            m.fn("as_array", 1, "(value: Json.Value)", R::result(R::array(named::json_value())),
                 {p.any}),
            m.fn("as_boolean", 1, "(value: Json.Value)", R::result_boolean(), {p.any}),
            m.fn("as_integer", 1, "(value: Json.Value)", R::result_integer(), {p.any}),
            m.fn("as_number", 1, "(value: Json.Value)", R::result_number(), {p.any}),
            m.fn("as_object", 1, "(value: Json.Value)", R::result(R::dict(named::json_value())),
                 {p.any}),
            m.fn("as_string", 1, "(value: Json.Value)", R::result_string(), {p.any}),
            m.fn("deserialize", 1, "(json: string)", R::result_any(), {p.string}),
            m.fn("equals", 2, "(a: string, b: string)", R::result_boolean(), {p.string, p.string}),
            m.fn("field", 2, "(value: Json.Value, key: string)", R::optional(named::json_value()),
                 {p.any, p.string}),
            m.fn("get", 2, "(json: string, key: string)", R::result_any(), {p.string, p.string}),
            m.fn("get_or", 3, "(json: string, path: string, default: T)", R::any_type(),
                 {p.string, p.string, p.any}),
            m.fn("get_path", 2, "(json: string, path: string)", R::result_any(),
                 {p.string, p.string}),
            m.fn("has_path", 2, "(json: string, path: string)", R::boolean_type(),
                 {p.string, p.string}),
            m.fn("index", 2, "(value: Json.Value, i: integer)", R::optional(named::json_value()),
                 {p.any, p.integer}),
            m.fn("is_array", 1, "(value: Json.Value)", R::boolean_type(), {p.any}),
            m.fn("is_boolean", 1, "(value: Json.Value)", R::boolean_type(), {p.any}),
            m.fn("is_null", 1, "(value: Json.Value)", R::boolean_type(), {p.any}),
            m.fn("is_number", 1, "(value: Json.Value)", R::boolean_type(), {p.any}),
            m.fn("is_object", 1, "(value: Json.Value)", R::boolean_type(), {p.any}),
            m.fn("is_string", 1, "(value: Json.Value)", R::boolean_type(), {p.any}),
            m.fn("is_valid", 1, "(json: string)", R::boolean_type(), {p.string}),
            m.fn("merge", 2, "(a: string, b: string)", R::result_string(), {p.string, p.string}),
            m.fn("minify", 1, "(json: string)", R::result_string(), {p.string}),
            m.fn("parse", 1, "(text: string)", R::result(named::json_value()), {p.string}),
            m.fn("parse_detailed", 1, "(text: string)",
                 R::result(named::json_value(), named::json_parse_error()), {p.string}),
            m.fn("pretty", 1, "(json: string)", R::result_string(), {p.string}),
            m.fn("remove_path", 2, "(json: string, path: string)", R::result_string(),
                 {p.string, p.string}),
            m.fn("serialize", 1, "(value: T)", R::string_type(), {p.any}),
            m.fn("serialize_pretty", 1, "(value: T)", R::string_type(), {p.any}),
            m.fn("set", 3, "(json: string, key: string, value: T)", R::result_string(),
                 {p.string, p.string, p.any}),
            m.fn("set_path", 3, "(json: string, path: string, value: T)", R::result_string(),
                 {p.string, p.string, p.any}),
            m.fn("to_string", 1, "(value: Json.Value)", R::string_type(), {p.any}),
        });
}

void register_csv_functions(std::vector<FunctionSpec>& specs, const ModuleBuilder& m,
                            const ParamShorthands& p) {
    append_specs(
        specs,
        {
            m.fn("count_rows", 1, "(csv: string)", R::result_integer(), {p.string}),
            m.fn("default_dialect", 0, "()", named::dialect()),
            m.fn("dialect", 2, "(delimiter: string, quote: string)", named::dialect(),
                 {p.string, p.string}),
            m.fn("deserialize", 1, "(csv: string)", R::result(R::array(R::array(R::string_type()))),
                 {p.string}),
            m.fn("deserialize_detailed", 1, "(csv: string)",
                 R::result(R::array(R::array(R::string_type())), named::csv_parse_error()),
                 {p.string}),
            m.fn("deserialize_records", 1, "(csv: string)",
                 R::result(R::array(R::dict(R::string_type()))), {p.string}),
            m.fn("deserialize_table", 1, "(csv: string)",
                 R::result(named::csv_table(), named::csv_parse_error()), {p.string}),
            m.fn("serialize_table", 1, "(table: Csv.Table)", R::result_string(),
                 {named::csv_table()}),
            m.fn("column", 2, "(table: Csv.Table, name: string)",
                 R::result(R::array(R::string_type())), {named::csv_table(), p.string}),
            m.fn("row", 2, "(table: Csv.Table, index: integer)",
                 R::result(R::dict(R::string_type())), {named::csv_table(), p.integer}),
            m.fn("select", 2, "(table: Csv.Table, names: array<string>)",
                 R::result(named::csv_table()), {named::csv_table(), p.array_string}),
            m.fn("filter_rows", 2,
                 "(table: Csv.Table, function: func(dictionary<string>) -> boolean)",
                 R::result(named::csv_table()), {named::csv_table(), p.func}),
            m.fn("header", 1, "(csv: string)", R::result(R::array(R::string_type())), {p.string}),
            m.fn("deserialize_with", 2, "(csv: string, options: Csv.Dialect | dictionary<string>)",
                 R::result(R::array(R::array(R::string_type()))), {p.string, p.any}),
            m.fn("read_file", 1, "(path: string)", R::result(R::array(R::dict(R::string_type()))),
                 {p.string}, Capability::FileSystem),
            m.fn("serialize", 1, "(rows: array<array<string>>)", R::result_string(), {p.array_any}),
            m.fn("serialize_records", 1, "(records: array<dictionary<string>>)", R::string_type(),
                 {p.array_any}),
            m.fn("serialize_with", 2,
                 "(rows: array<array<string>>, options: Csv.Dialect | dictionary<string>)",
                 R::result_string(), {p.array_any, p.any}),
            m.fn("write_file", 2, "(path: string, records: array<dictionary<string>>)",
                 R::result_boolean(), {p.string, p.array_any}, Capability::FileSystem),
        });
}

void register_xml_functions(std::vector<FunctionSpec>& specs, const ModuleBuilder& m,
                            const ParamShorthands& p) {
    append_specs(
        specs,
        {
            m.fn("add_child", 2, "(parent: xml, child: xml)", named::xml(), {p.xml, p.xml}),
            m.fn("add_comment", 2, "(node: xml, comment: string)", named::xml(), {p.xml, p.string}),
            m.fn("at", 2, "(node: xml, path: string)", R::result(named::xml()), {p.xml, p.string}),
            m.fn("attribute", 2, "(node: xml, name: string)", R::result_string(),
                 {p.xml, p.string}),
            m.fn("attributes", 1, "(node: xml)", R::dict_string(), {p.xml}),
            m.fn("child_count", 1, "(node: xml)", R::integer_type(), {p.xml}),
            m.fn("children", 1, "(node: xml)", R::array(named::xml()), {p.xml}),
            m.fn("children_by_tag", 2, "(node: xml, tag: string)", R::array(named::xml()),
                 {p.xml, p.string}),
            m.fn("deserialize", 1, "(xml: string)", R::result(named::xml()), {p.string}),
            m.fn("deserialize_detailed", 1, "(xml: string)",
                 R::result(named::xml_node(), named::xml_parse_error()), {p.string}),
            m.fn("deserialize_file", 1, "(path: string)", R::result(named::xml()), {p.string},
                 Capability::FileSystem),
            m.fn("element", 1, "(tag: string)", named::xml(), {p.string}),
            m.fn("escape", 1, "(value: string)", R::string_type(), {p.string}),
            m.fn("find", 2, "(node: xml, tag: string)", R::result(named::xml()), {p.xml, p.string}),
            m.fn("find_all", 2, "(node: xml, tag: string)", R::array(named::xml()),
                 {p.xml, p.string}),
            m.fn("find_all_descendants", 2, "(element: xml, tag: string)", R::array(named::xml()),
                 {p.xml, p.string}),
            m.fn("find_by_attribute", 3, "(node: xml, attribute: string, value: string)",
                 R::result(named::xml()), {p.xml, p.string, p.string}),
            m.fn("find_descendant", 2, "(element: xml, tag: string)", R::result(named::xml()),
                 {p.xml, p.string}),
            m.fn("from_dictionary", 2, "(tag: string, dictionary: dictionary)", named::xml(),
                 {p.string, p.dict_any}),
            m.fn("get_path", 2, "(element: xml, path: string)", R::result(named::xml()),
                 {p.xml, p.string}),
            m.fn("has_attribute", 2, "(node: xml, name: string)", R::boolean_type(),
                 {p.xml, p.string}),
            m.fn("has_child", 2, "(node: xml, tag: string)", R::boolean_type(), {p.xml, p.string}),
            m.fn("inner_text", 1, "(element: xml)", R::string_type(), {p.xml}),
            m.fn("is_leaf", 1, "(node: xml)", R::boolean_type(), {p.xml}),
            m.fn("is_valid", 1, "(xml: string)", R::boolean_type(), {p.string}),
            m.fn("remove_attribute", 2, "(node: xml, name: string)", named::xml(),
                 {p.xml, p.string}),
            m.fn("remove_child", 2, "(element: xml, index: integer)", R::result(named::xml()),
                 {p.xml, p.integer}),
            m.fn("replace_child", 3, "(element: xml, index: integer, new_child: xml)",
                 R::result(named::xml()), {p.xml, p.integer, p.xml}),
            m.fn("serialize", 1, "(node: xml)", R::string_type(), {p.xml}),
            m.fn("serialize_pretty", 1, "(node: xml)", R::string_type(), {p.xml}),
            m.fn("set_attribute", 3, "(node: xml, name: string, value: string)", named::xml(),
                 {p.xml, p.string, p.string}),
            m.fn("set_cdata", 2, "(node: xml, cdata: string)", named::xml(), {p.xml, p.string}),
            m.fn("set_tag", 2, "(node: xml, tag: string)", named::xml(), {p.xml, p.string}),
            m.fn("set_text", 2, "(node: xml, text: string)", named::xml(), {p.xml, p.string}),
            m.fn("tag", 1, "(node: xml)", R::string_type(), {p.xml}),
            m.fn("text", 1, "(node: xml)", R::result_string(), {p.xml}),
            m.fn("text_at", 2, "(node: xml, path: string)", R::result_string(), {p.xml, p.string}),
            m.fn("to_dictionary", 1, "(node: xml)", R::dict_string(), {p.xml}),
            m.fn("to_node", 1, "(node: xml)", named::xml_node(), {p.xml}),
            m.fn("unescape", 1, "(value: string)", R::result_string(), {p.string}),
            m.fn("write_file", 2, "(path: string, node: xml)", R::result_void(), {p.string, p.xml},
                 Capability::FileSystem),
        });
}

} // namespace luma::stdlib::detail
