...
void register_socket_functions(std::vector<FunctionSpec>& specs, const ModuleBuilder& m,
    ...
    append_specs(
        specs,
        {
            ...
            m.fn("connect_timeout", 3, "(host: string, port: integer, timeout_ms: integer)",
                 R::result(named::socket()), {p.string, p.integer, p.integer}),
            m.fn("connect_timeout_typed", 3, "(host: string, port: integer, timeout_ms: integer)",
            ...
            m.fn("receive", 2, "(s: socket, max_bytes: integer)", R::result_string(),
                 {p.socket, p.integer}),
            m.fn("receive_all", 1, "(s: socket)", R::result_string(), {p.socket}),
            m.fn("receive_bytes", 2, "(s: socket, max: integer)", R::result(R::array_integer()),
                 {p.socket, p.integer}),
            m.fn("receive_line", 1, "(s: socket)", R::result_string(), {p.socket}),
            ...
            m.fn("send_all", 2, "(s: socket, data: string)", R::result_boolean(),
                 {p.socket, p.string}),
            m.fn("send_all_typed", 2, "(s: socket, data: string)",
                 R::result(R::boolean_type(), named::socket_error()), {p.socket, p.string}),
            m.fn("send_bytes", 2, "(s: socket, bytes: array<integer>)", R::result_integer(),
